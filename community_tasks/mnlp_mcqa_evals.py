...
def mmlu_harness(line, task_name: str = None):
    ...
    #prompt += "Answer:"
    gold_idx = LETTER_INDICES.index(line["letter"])
...
task = LightevalTaskConfig(
    ...
    suite=["community"],
    hf_subset="",
    hf_repo="publication-charaf/MNLP_M3_mcqa_dataset",  # Change the repo name to the evaluation dataset that you compiled
    ...
    stop_sequence=None,
    trust_dataset=True,
    limited_num_samples=1000,  # Set to 0 to use all samples, specify a number to limit the number of samples for debugging purposes
)
